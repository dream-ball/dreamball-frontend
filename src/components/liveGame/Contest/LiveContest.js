...
export default function LiveContest() {
    ...
    const fetchMatchData = useCallback(async () => {
        try {
            ...
            matchUrl.pathname = `/api/live_match/${match_id}`;
            ...
            const match_info = await matchInfo_response.json()

            if (!matchInfo_response.ok) {
                setMatchInfo([]);
                setOverInfo([]);
            } else {
                setMatchInfo(match_info["matchInfo"][0]);
                if (!match_info["oversData"][0].status) {
                    setOverInfo(match_info["oversData"]);
            ...
            if (!match_response.ok) throw new Error(match_data.msg);
            if (!contest_response.ok) throw new Error(contest_data.msg);

            setMatchData(JSON.parse(match_data.match_data));
            SetContest(contest_data);
            setLoading(false);
        } catch (error) {
    ...
    return (
        <>
            {loading ? display_loading(true) : error ? <>{error.toLowerCase() === "invalid or expired token" ? <> {display_error(error)}{navigate('/login')}</> : display_error(error)}{display_loading(false)}</> : <>
                ...
                <div className="live_match_status">
