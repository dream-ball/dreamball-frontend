...
import UserInputTable from "./Contest/UserInputTable";
const LiveGame = () => {
    ...
    useEffect(() => {
        const fetchOversData = async () => {
            try {
                server.pathname = `/api/live_match/${match_id}/info/`;
                ...
                if (!jsonData || !jsonData.oversData || !jsonData.matchInfo || !jsonData.oversData) {
                    throw new Error(jsonData.error || "Error Occured")
                }
        ...
        const fetchUserOversData = async () => {
            try {
                server.pathname = `/api/live/user/history/${match_id}`;
                const response = await fetch(server, {
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": localStorage.getItem("auth_token") || "",
    ...
    useEffect(() => {
        const fetchSelectedOptions = async (open_over) => {
            const data = {
                match_id: open_over[0].match_id,
                over_number: open_over[0].over_number,
                innings: open_over[0].innings
            };
            try {
                server.pathname = '/api/user/selected_options/'
                const response = await fetch(server, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
        ...
        if (openOver.length > 0 && !hasFetched) {
            fetchSelectedOptions(openOver);
            setHasFetched(true);
        }
    }, [openOver, hasFetched])
    ...
    return (
        <div className="max-w-3xl mx-auto p-5">
            {loading ? (
                ...
                <>{error.toLowerCase() === "invalid or expired token" ? <> {display_error(error)}{navigate('/login')}</> : display_error(error)}{display_loading(false)}</>
            ) : (
                <>
                    {(
                        <>
                            ...
                            <div className="live_match_card_con">
                                <LiveMatchCard match_data={matchData} overs_data={oversData} match_info={matchInfo} />
                            </div>

                            <UserInputTable openOver={openOver} selectedOptions={selectedOptions} setHasFetched={setHasFetched} />
                            <div className="live_game_con">
                                <div className="user_block_options">
                                    <div className="block">
                                        <div
                                            className={`match_overs_data ${activeBlock === "overs" ? "active" : ""}`}
                                            onClick={() => setActiveBlock("overs")}
                                        >
                                            Over History
                                        </div>
                                        <div
                                            className={`user_overs_data ${activeBlock === "history" ? "active" : ""}`}
                                                ...
                                                <table className="w-full border-collapse border border-gray-300">
                                                    <thead>
                                                        <tr className="bg-gray-200">
                                                            ...
                                                            <th className="border p-2">Dots</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        {!userInputs.length ? <></> : <>
                                                            {userInputs[0][2].map((over, index) => (
                                                                <tr key={`${index}`} className="border">
                                                                    <td className="border p-2">{over?.over_number}</td>
                                                                    <td className="border p-2">{over?.run || "-"}</td>
                                                                    <td className="border p-2">{over?.four || "-"}</td>
                                                                    <td className="border p-2">{over?.six || "-"}</td>
                                                                    <td className="border p-2">{over?.wicket || "-"}</td>
                                                                    <td className="border p-2">{over?.dot || "-"}</td>
